...
def subseqs(s):
    ...
    if s == []:
        ...
        all_except_first = subseqs(s[1:])
        return all_except_first + insert_into_all(s[0], all_except_first)
def inc_subseqs(s):
    """Assuming that S is a list, return a nested list of all subsequences
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_points, current_points = 0, 0
    def check(assignment, points):
        nonlocal total_points, current_points
        total_points += class_assignments[assignment]
        current_points += points
        rate = current_points / total_points
        return rate
    return check
