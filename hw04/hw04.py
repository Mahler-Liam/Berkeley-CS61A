...
def balanced(m):
    ...
    if is_planet(m):
        return True
    m_left, m_right = end(left(m)), end(right(m))
    return balanced(m_left) and balanced(m_right) and total_weight(m_left) * length(left(m)) == total_weight(m_right) * length(right(m))
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        m_left, m_right = end(left(m)), end(right(m))
        return tree(total_weight(m), [totals_tree(m_left), totals_tree(m_right)])
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 10000000) # Replace this line!
    r2 = interval(1, 10000000) # Replace this line!
    return r1, r2
